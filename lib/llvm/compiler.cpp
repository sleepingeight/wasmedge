...
Expect<void> Compiler::compile(const AST::FunctionSection &FuncSec,
  ...
  for (auto [T, F, Code] : Context->Functions) {
    ...
    FunctionCompiler FC(*Context, F, Locals,
                        ...
                        Conf.getRuntimeConfigure().isLazyJIT());
...
Expect<void> Compiler::compileFunctionBody(uint32_t LocalFuncIndex) noexcept {
  ...
  if (GlobalFuncIndex >= Context->Functions.size()) {
    spdlog::error("LazyJIT: function index {} out of range"sv, LocalFuncIndex);
    return Unexpect(ErrCode::Value::IllegalPath);
  }

  auto &[T, F, Code] = Context->Functions[GlobalFuncIndex];
  if (!Code) {
    spdlog::error("LazyJIT: cannot compile import function {}"sv,
                  LocalFuncIndex);
    return Unexpect(ErrCode::Value::IllegalPath);
  }
  ...
  if (F.Fn.countBasicBlocks() > 0) {
    spdlog::debug("LazyJIT: function {} already compiled"sv, LocalFuncIndex);
    return {};
  }

  spdlog::debug("LazyJIT: compiling function {}"sv, LocalFuncIndex);
...
Expect<Data>
Compiler::compileInfrastructure(const AST::Module &Module) noexcept {
  // Check the module is validated.
  if (unlikely(!Module.getIsValidated())) {
    spdlog::error(ErrCode::Value::NotValidated);
    return Unexpect(ErrCode::Value::NotValidated);
  ...
  std::unique_lock Lock(Mutex);
  spdlog::info("compile infrastructure start (lazy JIT mode)"sv);
  ...
  spdlog::info("infrastructure compilation done (lazy JIT mode)"sv);
...
Expect<Data> Compiler::compileFunction(const AST::Module &Module,
  ...
  std::unique_lock Lock(Mutex);
  spdlog::debug("compile function {} start"sv, FuncIndex);
  ...
  spdlog::info("verify function {} start"sv, FuncIndex);
  LLModule.verify(LLVMPrintMessageAction);

  spdlog::info("optimize function {} start"sv, FuncIndex);
  auto &TM = D.extract().TM;
  {
    auto Triple = LLModule.getTarget();
    auto [TheTarget, ErrorMessage] = LLVM::Target::getFromTriple(Triple);
  ...
  spdlog::debug("compile function {} done"sv, FuncIndex);
  return Expect<Data>{std::move(D)};
}
